...
# Install Open MPI
OMPI_VERSION=$(get_component_config "ompi" | jq '.version')
tdnf install openmpi-${OMPI_VERSION} -y
...
OMPI_ARCH=$(rpm -q --queryformat '%{arch}' openmpi)

# Create symlinks for modulefiles
ln -s /usr/share/Modules/modulefiles/mpi/mvapich2-${MVAPICH2_VERSION} /usr/share/Modules/modulefiles/mpi/mvapich2
ln -s /usr/share/Modules/modulefiles/mpi/openmpi-${OMPI_ARCH} /usr/share/Modules/modulefiles/mpi/openmpi
